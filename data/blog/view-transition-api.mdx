...
react-router v6 推荐使用 [Data APIs](https://reactrouter.com/en/main/routers/picking-a-router)，因此直接支持了 [`unstable_viewtransition`](https://reactrouter.com/en/main/components/link#unstable_viewtransition)
...
在 `startViewTransition` 回调中进行 vue-router 组件切换，同时返回 `nextTick()` promise，`startViewTransition` 就会等待 `nextTick()` 兑现后再去截取新帧，然后正确应用 view transition 过渡动画
