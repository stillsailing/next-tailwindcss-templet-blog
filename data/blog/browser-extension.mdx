...
如果必须使用的话，并不是没有办法，需要创建 sandbox 然后通过 iframe 加载并通信，但在 sandbox 中被禁止调用诸如 `chrome.*` 的任何浏览器 API
...
还是举例说明，比如想在插件中代理请求 mock 代替接口返回，但是需要支持脚本动态 mock 数据

#### 开发和使用注意

1. 权限最小化原则：应只请求完成其功能所需的最小权限，避免请求不必要的权限。
2. 谨慎授权：安装前仔细查看其请求的权限列表，评估其合理性
